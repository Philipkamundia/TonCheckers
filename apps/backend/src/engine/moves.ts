 ...
 *
 * Russian checkers rules enforced here:
 * - Mandatory capture: if any jump is available, ONLY jumps are legal
 * - Maximum capture: must take the sequence that captures the most pieces
 * - Multi-jump chains: must be completed in one turn
 * - Backward capture: allowed for ALL pieces (regular and kings)
 * - Flying kings: kings slide any number of squares diagonally
