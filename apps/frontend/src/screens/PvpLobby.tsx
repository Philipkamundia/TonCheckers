...
export function PvpLobby() {
  ...
  useEffect(() => {
    return showBackButton(() => {
      if (inQueue) matchmakingApi.cancel().catch(() => null);
      navigate('/');
    });
  }, [inQueue]);
  ...
  useEffect(() => {
    const unsubs = [
      on<{ gameId: string; stake: string; opponentElo: number; countdownMs: number }>('mm.found', (data) => {
        haptic.success();
        setPendingLobby({ gameId: data.gameId, stake: data.stake, opponentElo: data.opponentElo });
        hideMainButton();
        navigate(`/lobby/${data.gameId}`);
      }),
      on<{ gameId: string; resolvedStake: string; originalStake: string }>('mm.stake_adjusted', (data) => {
        haptic.warning();
  ...
  async function handleFindMatch() {
    ...
    try {
      ...
      setError((e as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed to join queue');
      haptic.error();
    } finally {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>PvP — Choose Stake</h2>
      <p style={styles.balance}>Available: {availableTON.toFixed(2)} TON</p>
      ...
      {!inQueue ? (
        <>
          <div style={styles.presets}>
            {PRESET_STAKES.map(s => (
              <button
                key={s}
                style={{ ...styles.preset, ...(stake === s && !customStake ? styles.presetActive : {}) }}
                onClick={() => { setStake(s); setCustomStake(''); haptic.selection(); }}
              >
                {s} TON
              </button>
            ))}
          ...
          <input
            ...
            step="0.1"
          />
          {!canAfford && <p style={styles.error}>Insufficient balance</p>}
        ...
        <div style={styles.queueBox}>
          ...
          <p style={styles.searching}>Searching for opponent…</p>
          <p style={styles.stakeLabel}>Stake: {finalStake} TON</p>
